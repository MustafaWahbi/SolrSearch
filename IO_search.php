...
if(isset($_REQUEST['category'])&&!empty($_REQUEST['category']))
    if($filter == null ){
        $multi_category_to_array=explode(',',urldecode($_REQUEST['category']));
...
if(isset($_REQUEST['min_price'])&&isset($_REQUEST['max_price']))($filter == null ? $filter.="fq=price:[".urlencode($_REQUEST['min_price'])."%20TO%20".urlencode($_REQUEST['max_price'])."]":$filter.="&fq=price:[".urlencode($_REQUEST['min_price'])."%20TO%20".urlencode($_REQUEST['max_price'])."]");
...
else*/ products_search($_REQUEST['term'],$_REQUEST['count'],$_REQUEST['page'],$_REQUEST['order'],$filter);
...
///////////////////////////////////////////////////// full product search
function products_search($term,$count=24,$page=1,$order,$filter){
    ...
    if($order != ""){
        /* fill the filters*/
        products_search_filters_filler($term,$order);
        /* pagination */
        $fullLink=Solr_url."select?defType=dismax&indent=on&q=".$name_query."&rows=".$limit."&start=".$start."&stats=true&stats.field=price&sort=price%20".$order."&wt=json&fq=parent_id:0".$isInStock."&mm=4&".$filter;
    }
    else {
        /* fill the filters*/
        products_search_filters_filler($term,$order);
        /* pagination */
        $fullLink=Solr_url."select?defType=dismax&indent=on&q=".$name_query."&rows=".$limit."&start=".$start."&stats=true&stats.field=price&wt=json&fq=parent_id:0".$isInStock."&mm=4&".$filter;
    }
...
//var_dump($fullLink); die();
    ...
    $full_final_result['label']=$name_query;
    ...
    if($main_result['response']['numFound'] > 0)
    {
        ...
        foreach ($main_result['response']['docs'] as $array_index => $doc)
        {
            ...
            if($doc['type_id'] ==  "simple"){
                ...
                $childLink=Solr_url."select?defType=dismax&indent=on&q=parent_id:".$doc['id']."&rows=50&wt=json".$isInStock;
        ...
        if($order != "")$fullLink=Solr_url."spell?indent=on&q=".$name_query."&rows=".$limit."&start=".$start."&stats=true&stats.field=price&sort=price%20".$order."&wt=json&fq=parent_id:0".$isInStock;
        else $fullLink=Solr_url."spell?indent=on&q=".$name_query."&rows=".$limit."&start=".$start."&stats=true&stats.field=price&wt=json&fq=parent_id:0".$isInStock;
        ...
        if(count($suggestions)){
            ...
            $name_query=str_replace ( ' ', '%20', $suggestions[0]['word'] );

            if($order != "")$fullLink=Solr_url."select?defType=dismax&indent=on&q=".$name_query."&rows=".$limit."&start=".$start."&stats=true&stats.field=price&sort=price%20".$order."&wt=json&fq=parent_id:0".$isInStock."&".$filter;
            else $fullLink=Solr_url."select?defType=dismax&indent=on&q=".$name_query."&rows=".$limit."&start=".$start."&stats=true&stats.field=price&wt=json&fq=parent_id:0".$isInStock."&".$filter;
            ...
            if($main_result['response']['numFound'] > 0)
            {
                ...
                foreach ($main_result['response']['docs'] as $array_index => $doc)
                {
                    ...
                    if($doc['type_id'] ==  "simple"){
                        ...
                        foreach ($children['response']['docs'] as $index => $child)
                        {
                            ...
                            {
                                $colors[]=$child['color'];
                                $full_final_result_colors[]=$child['color'];
                            }
                            if ((!in_array($child['size'], $sizes)) && ($child['size'] != "0"))
                            {
                                $sizes[]=$child['size'];
                                $full_final_result_sizes[]=$child['size'];
                            }
                ...
                //$full_final_result_sizes_remove_duplicate = array_map("unserialize", array_unique(array_map("serialize", $full_final_result_sizes)));
                //$full_final_result_colors_remove_duplicate = array_map("unserialize", array_unique(array_map("serialize", $full_final_result_colors)));
                $full_final_result_brands_remove_duplicate = array_map("unserialize", array_unique(array_map("serialize", $full_final_result_brands)));
                //$full_final_result_types_remove_duplicate = array_map("unserialize", array_unique(array_map("serialize", $full_final_result_types)));
...
function products_search_filters_filler($term,$order){
    ...
    if($order != "")$fullLink=Solr_url."select?defType=dismax&indent=on&q=".$name_query."&rows=2000&start=0&stats=true&stats.field=price&sort=price%20".$order."&wt=json&fq=parent_id:0".$isInStock."&mm=4";
    else $fullLink=Solr_url."select?defType=dismax&indent=on&q=".$name_query."&rows=2000&start=0&stats=true&stats.field=price&wt=json&fq=parent_id:0".$isInStock."&mm=4";
    ...
    if($main_result['response']['numFound'] > 0)
    {
        ...
        if($order != "")$fullLink=Solr_url."spell?indent=on&q=".$name_query."&stats=true&stats.field=price&sort=price%20".$order."&wt=json&fq=parent_id:0".$isInStock."&mm=4";
        else $fullLink=Solr_url."spell?indent=on&q=".$name_query."&stats=true&stats.field=price&wt=json&fq=parent_id:0".$isInStock."&mm=4";
        ...
        if(count($suggestions)){
            ...
            if($order != "")$fullLink=Solr_url."select?defType=dismax&indent=on&q=".$name_query."&rows=20000&start=0&stats=true&stats.field=price&sort=price%20".$order."&wt=json&fq=parent_id:0".$isInStock."&mm=4";
            else $fullLink=Solr_url."select?defType=dismax&indent=on&q=".$name_query."&rows=20000&start=0&stats=true&stats.field=price&wt=json&fq=parent_id:0".$isInStock."&mm=4";
            ...
            if($main_result['response']['numFound'] > 0)
            {
                ...
                foreach ($categories as $category_key =>$category ){
                    $full_final_result_categories_remove_duplicate[$category_counter]['id']=$category_key;
                    $full_final_result_categories_remove_duplicate[$category_counter]['label']=$category;
                    $full_final_result_categories_remove_duplicate[$category_counter]['count']=$category;
                    $category_counter++;
                }
