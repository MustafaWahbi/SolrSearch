...
if(isset($_REQUEST['size'])&&!empty($_REQUEST['size']))
{
    ...
    $size_filter="fq=size_value:".urlencode($multi_size_to_string);*/
    $multi_size_to_array=explode(',',urldecode($_REQUEST['size']));
    $multi_size_to_string=implode('%20',$multi_size_to_array);
    $size_filter="&fq=size_value:(".$multi_size_to_string.")";
}
...
if(isset($_REQUEST['order'])&&!empty($_REQUEST['order'])){
    $order="&sort=final_price%20".$_REQUEST['order'];
    //&sort=final_price%20".$order."
}
else $order="";
...
else products_search($_REQUEST['count'],$_REQUEST['page'],$order,$filter,$category_filter,$just_category_filter);
...
function products_search($count=24,$page=1,$order,$filter,$category_filter,$just_category_filter){
    ...
    /* pagination */
    if($size_filter == Null)$fullLink=Solr_url."select?indent=on&q=*&rows=".$limit."&start=".$start."&stats=true&stats.field=price".$order."&wt=json&fq=status:1&fq=parent_id:0".$isInStock."&".$filter;
    else {
        //http://solrsearch.tools.kskdigital.com:8983/solr/magento/select?facet.field=parent_id&facet.query=size_value:(408%20409%20407)&facet=on&fl=parent_id&fq=size_value:(408%20409%20407)&indent=on&q=*&rows=0&wt=json
        /*
         * get all the parent product for those child with specific colors
         * */
        $colorLink=Solr_url."select?indent=on&q=*&rows=".$limit."&start=".$start."&stats=true&stats.field=price".$order."&wt=json&fl=parent_id&fq=status:1".$size_filter."&".$isInStock."&".$filter;
        $color_result=json_decode(solr_curl($colorLink),true);
        $parent_ids=[];
        foreach ($color_result['response']['docs'] as $array_index => $doc)
        {
            if(!in_array($doc['parent_id'],$parent_ids))$parent_ids[]=$doc['parent_id'];
        }

        $fq_parent_ids=implode("%20",$parent_ids);///////////////// parent ids
        //var_dump($fq_parent_ids);
        $fullLink=Solr_url."select?indent=on&q=*&rows=".$limit."&start=".$start."&stats=true&stats.field=price".$order."&wt=json&fq=status:1&fq=id:(".$fq_parent_ids.")&".$isInStock."&".$filter;
    }
...
function products_search_filters_filler($order,$filter){
    ...
    ///////////if($order != "")$fullLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fq=parent_id:0".$isInStock."&".$filter;

    /********** old*******/
    //if($size_filter == Null)$fullLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fq=status:1&fq=parent_id:0".$isInStock."&".$filter;
    //else $fullLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fq=status:1".$size_filter."&".$isInStock."&".$filter;
    ...
    if($size_filter == Null)$fullLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fq=status:1&fq=parent_id:0".$isInStock."&".$filter;
         ...
         * get all the parent product for those child with specific colors
         * */
        $colorLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fl=parent_id&fq=status:1".$size_filter."&".$isInStock."&".$filter;
        $color_result=json_decode(solr_curl($colorLink),true);
        $parent_ids=[];
        foreach ($color_result['response']['docs'] as $array_index => $doc)
        {
        ...
        $fq_parent_ids=implode("%20",$parent_ids);///////////////// parent ids
        //var_dump($fq_parent_ids);
        $fullLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fq=status:1&fq=id:(".$fq_parent_ids.")&".$isInStock."&".$filter;
    }
    /**************/
...
function products_search_filters_category($order,$just_category_filter){
    ...
    // if($order != "")$fullLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fq=status:1&fq=parent_id:0".$isInStock."&".$just_category_filter;

    if($size_filter == Null)$fullLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fq=status:1&fq=parent_id:0".$isInStock."&".$just_category_filter;
    else $fullLink=Solr_url."select?indent=on&q=*&rows=2000&start=0&stats=true&stats.field=price".$order."&wt=json&fq=status:1".$size_filter."&".$isInStock."&".$just_category_filter;
